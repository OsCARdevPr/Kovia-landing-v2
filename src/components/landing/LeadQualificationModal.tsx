...
const FORM_ID = "lead-qualificator";
...
interface FormOpenDetail {
  readonly cta_location?: string;
...
export function LeadQualificationModal() {
  ...
  const leadSessionIdRef = useRef<string | null>(null);

  useEffect(() => {
    const openModal = (event: Event) => {
      const detail = (event as CustomEvent<FormOpenDetail>).detail;
      const leadSessionId = createEventId();
      leadSessionIdRef.current = leadSessionId;

      pushDataLayer({
        event: "cta_click",
        event_id: createEventId(),
        cta_location: detail?.cta_location ?? "unknown",
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (!leadSessionIdRef.current) {
        leadSessionIdRef.current = createEventId();
      }
      ...
      pushDataLayer({
        ...
        form_id: FORM_ID,
        form_url: EMBED_URL,
        form_origin: allowedOrigin,
        source: "lead_qualification_modal",
        status: "success",
        lead_session_id: leadSessionIdRef.current,
      });
